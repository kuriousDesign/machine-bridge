/**
 * OPC UA Manager - Step-by-Step Breakdown
 * 
 * OVERVIEW:
 * This class manages real-time communication between web clients (via Socket.IO) and an OPC UA server.
 * It acts as a bridge, allowing web applications to read/write industrial automation data.
 * 
 * INITIALIZATION FLOW:
 * 1. Creates a dedicated heartbeat connection to the OPC UA server for keep-alive monitoring
 * 2. Sets up a timer that reads from PLC heartbeat node and writes to HMI heartbeat node every 500ms
 * 3. Listens for incoming Socket.IO connections from web clients
 * 
 * PER-CLIENT CONNECTION FLOW:
 * 4. When a web client connects via Socket.IO:
 *    - Creates a new OPC UA client connection specifically for that client
 *    - Establishes an OPC UA session for communication
 *    - Sets up monitoring subscriptions for configured nodes (real-time data streaming)
 *    - Registers event handlers for various client operations
 * 
