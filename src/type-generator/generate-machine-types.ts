...
import { OpcuaEnumEntry, readEnumMetadata } from './enum-reader';
...
const DEFAULT_OUTPUT_PATH = path.resolve(__dirname, '../../type-detection/types.ts');
const DEFAULT_TAGS_OUTPUT_PATH = path.resolve(__dirname, '../../type-detection/machine-tags.ts');
...
function toMachineSpecificOutputPath(machineId: string): string {
    return path.resolve(__dirname, `../../type-detection/machine-${machineId}-types.ts`);
}

function toMachineSpecificTagsOutputPath(machineId: string): string {
    return path.resolve(__dirname, `../../type-detection/machine-${machineId}-tags.ts`);
}
...
async function buildSchemaFromDataTypeDefinition(
    ...
    if (isEnumDefinition(definition)) {
        const enumEntries = await readEnumMetadataByTypeName(session, dataTypeName);
        return {
            browseName,
            dataTypeName,
            dataTypeNodeId,
...
async function buildSchemaTree(
    ...
    if (childReferences.length === 0) {
        return {
            ...
            dataTypeName,
            dataTypeNodeId,
            enumEntries: await readEnumMetadataByTypeName(session, dataTypeName) ?? undefined,
            isArray,
            kind: 'leaf',
            nodeId,
